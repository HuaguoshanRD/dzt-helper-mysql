...
function zipFileWith7zip(zipfilename,resourcefilename,persize){                          //压缩文件
    return new Promise(
        function(resolve,reject){
            var exec = require('child_process').exec;
            var cmd = '7z a ' + zipfilename +' '+ resourcefilename + ' '+persize;
            cmd =cmd.replace(/'/g,"");
            exec(cmd, {cwd:'./public/download'},function callback(error, stdout, stderr) {
                if(error){
                    return reject(error)
                }else{
                    return resolve("zip sql success");
                }
            });
    });
}
...
function sendHttpRequest(url){
    return new Promise(
        function(resolve,reject){
            http.get("http://localhost:3000/service/exportFromMysql", function (response) {
...
//    user:'root',
//    password:'123456',
//    sqlFilePath:'./rds_201707240430.sql',
