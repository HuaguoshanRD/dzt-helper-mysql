!function() {
	...
	['bold', 'underline', 'italic', 'inverse', 'grey', 'yellow', 'red', 'green', 'blue', 'white', 'cyan', 'magenta'].forEach(function(style) {
		try{
			Object.defineProperty(String.prototype, style, {
				get: function () {
					return argv.has('no-colors') ? this : stylize(this, style);
				}
				, configurable: true
			});
		} catch(e) {}
	...
	module.exports = new Class({
		...
		// private dir
		, _dir: function(data, margin, name, first, knownObjects ) {
			var fnname = name;
			...
			switch (typeof data) {
				case 'object':
					// null
					if (data === null) {
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + 'null'.white);
					}
					...
					// date
						...
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + data.toString().white);
					}
					...
					// set
						...
						// convert to array
						this._dir(Array.prototype.slice.call(data), margin, (fnname ? (fnname+' [Set]') : '[Set]'), first, knownObjects);
					}

					// weak set
					else if (type.weakSet(data)) {
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + ('Weak Set').grey);
					}
					...
					// weak map
						...
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + ('Weak Map').grey);
					}


					// buffer
					else if (Buffer.isBuffer(data)) {
						var result =  this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name;
						...
						if (knownObjects.indexOf(data) >= 0) {
							return console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + '[circular]'.grey);
						}
						else {
						...
						if (margin > 20) return console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + ': '.grey + (isArray ?  '(' + data.length + '):[...]' : '{ ... }').grey + ' // max inspection depth reached'.grey);

						if (i === 0) {
							return console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + (isArray ? '(' + data.length + '):[]' : '{}').grey);
						}

						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + (isArray ? '(' + data.length + '):[' : '{').grey);
						first = true;
						while(i--) {
							if (k > 400) {
							...
							if (Object.prototype.hasOwnProperty.call(data, keys[l - i - 1 ])) this._dir(data[keys[l - i - 1 ]], margin + 1, (isArray ? '' +  keys[l - i - 1 ]  : keys[l - i - 1 ]), first, knownObjects);
				...
				case 'string':
					if (data.length > 400) {
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + '"'.white + data.substr(0, 400).green + '..."'.white + ' (omitted '.grey + (data.length - 400 + '').grey + ')'.grey);
					}
					else {
						console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + '"'.white + data.green + '"'.white);
					}
					break;

				case 'number':
					console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + (data + '').blue.bold);
					break;

				case 'boolean':
					console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + data.toString().yellow);
					break;

				case 'function':
					console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + ('function ' + (fnname || '')).grey);
					break;

				case 'symbol':
					console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + data.toString().green);
					break;

				default:
					console.log(this._pad('', margin * 4, ' ') + (! first ? ', ' : '').grey + name + data);
					break;
		...
		, trace: function trace(err, skip) {
			...
			if (err && err.stack) {
				...
				while(i--) {
					...
					if (current) {
						console.log(this._pad(current[2].replace(project.root, '/'), 30, ' ').yellow + this._pad(current[3], 5, ' ').white + ':'.grey + this._pad(current[4], 4, ' ', true).grey + current[1].white );
					}
				}
